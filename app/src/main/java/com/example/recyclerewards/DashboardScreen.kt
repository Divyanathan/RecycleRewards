...
@Composable
fun SubCategoriesGrid(subCategories: List<SubCategory>) {
    LazyVerticalGrid(
        columns = GridCells.Fixed(3), // Displayitems in a grid with 2 columns
        modifier = Modifier.padding(8.dp)
    ) {
        items(subCategories.size) { index ->
            SubCategoryItem(subCategories[index])
        }
    }
}
